...
const PRINT_WINDOW_NAME = 'superintelligent_pdf_print_en';
          ...
          <div class="method-panel">
            ...
            <div class="levels-table-wrap">
              <table class="levels-table">
                <caption class="sr-only">Levels for economic impact from AI in knowledge work</caption>
                <thead>
                  <tr>
...
<head>
  ...
  <title>${currentPdfFileName || 'ai-decision-brief'}</title>
  <style>html,body{margin:0;padding:0;background:#fff;}</style>
  <link rel="stylesheet" href="${toAbsoluteUrl('/en/index.css')}" />
</head>
<body>
  ${pdfStage.outerHTML}
</body>
...
function openPrintWindow() {
  ...
  if (!printWindow) {
    failPrintFlow('The print window could not be opened. The browser likely blocked it. Allow pop-ups for this site and try again.', {
  ...
  if (printWindow.closed) {
    failPrintFlow('The print window closed immediately and could not be used for PDF export.', {
...
async function printPdfStage(pdfStage) {
  ...
  try {
    ...
    failPrintFlow('The print window opened, but the document could not be prepared for PDF export.', {
  ...
  try {
    ...
    failPrintFlow('The print window opened, but the content did not become ready for printing in time.', {
  ...
  try {
    ...
    failPrintFlow('The print window opened, but the print dialog could not be started automatically. Use the browser print action in the opened window.', {
      reason: 'print_failed'
    }, error);
    return false;
  }
...
async function exportDecisionPdf() {
  ...
  try {
    ...
    failPrintFlow('PDF export failed before the print window could be started.', {
